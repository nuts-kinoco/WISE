...
├── MetadataField（作品情報の各フィールド、FTS5対象）
├── Job（非同期処理の実行単位）
├── EventLog（状態変化履歴）
...
AssetはPhysicalFileのDB上の表現である。各AssetはWorkの中で「どういう役割を担うか」を `AssetRole` で表現する。
...
SQLite FTS5を使用して、全MetadataFieldを横断的に検索できる仮想テーブルを管理する。
...
Collectionは単一テーブルで全種別を管理し、v2でクロスメディア対応する。
